...
export class SimpleSUDTBuilder extends Builder {
  fee: Amount;
  sendCKBAmount: Amount;

  constructor(
    private sudt: SUDT,
    private address: Address,
    private amount: Amount,
    private provideCapacity?: boolean,
    feeRate?: number,
    collector?: Collector
  ) {
    super(feeRate, collector);
    this.fee = new Amount('0');
    this.sendCKBAmount = new Amount('0');
  }
  ...
  async build(): Promise<Transaction> {
    ...
    let receiverInputCell = null;
    let receiverOutputCell = null;
    ...
    if (
      !this.provideCapacity &&
      (!receiverSUDTCells || receiverSUDTCells.length === 0)
    ) {
      throw new Error('The receiver has no sudt cell');
    }

    if (receiverSUDTCells.length > 0) {
      receiverInputCell = receiverSUDTCells[0];
      receiverOutputCell = receiverInputCell.clone();

      receiverOutputCell.setSUDTAmount(
        this.amount.add(receiverOutputCell.getSUDTAmount())
      );
    } else {
      this.sendCKBAmount = new Amount('142');
      receiverOutputCell = new Cell(
        this.sendCKBAmount,
    ...
    if (receiverInputCell) inputCells.push(receiverInputCell);
    outputCells.unshift(receiverOutputCell);

    let tx = this.rectifyTx(inputCells, outputCells);

    const availableCKBFee = senderInputCKBSum.sub(minSenderOccupiedCKBSum);

    // Second step:  if sudt cell can not pay the transaction fee and usdt cell capacity, fetch pure ckb cells to pay the fee.
    if (this.fee.add(this.sendCKBAmount).gt(availableCKBFee)) {
      const unspentCKBCells = await this.collector.collect(
        PWCore.provider.address,
        this.fee
          .add(this.sendCKBAmount)
          .sub(availableCKBFee)
      ...
      let neededCKBAmount = this.fee
        .add(this.sendCKBAmount)
        .sub(availableCKBFee);
      for (const ckbCell of unspentCKBCells) {
        inputCells.push(ckbCell);

        if (neededCKBAmount.gt(ckbCell.capacity)) {
          neededCKBAmount = neededCKBAmount.sub(ckbCell.capacity);
        } else if (
          neededCKBAmount.add(Builder.MIN_CHANGE).lt(ckbCell.capacity)
        ) {
          // rest capacity big than 61 CKBytes, append a change cell to outputs
          const changeCell = ckbCell.clone();
          changeCell.capacity = changeCell.capacity.sub(neededCKBAmount);
          outputCells.push(changeCell);

          neededCKBAmount = new Amount('0');
        } else {
          // rest capacity less than 61 CKBytes, merge the rest cpacity to sender's sudt cell
