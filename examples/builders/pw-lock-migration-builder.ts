import {Address, Amount, AmountUnit, AddressType, Builder, Cell, RawTransaction, Transaction, WitnessArgs } from '@lay2/pw-core';
...
export class PWLockMigrationBuilder extends Builder {
  ...
  constructor(sourceAddressString: string, destinationAddressString: string, amount: Amount) {
    super();
    this.sourceAddress = new Address(sourceAddressString, AddressType.ckb);
    this.sourceAddressString = sourceAddressString;
    this.destinationAddress = new Address(destinationAddressString, AddressType.ckb);
  ...
  async build(fee: Amount = Amount.ZERO): Promise<Transaction> {
    ...
    const outputCell = new Cell(this.amount, this.destinationAddress.toLockScript());

    // Collect the input cells from the source address.
    const neededAmount = this.amount.add(this.sourceAddress.minPaymentAmount()).add(fee); // send amount + minimum change cell capacity + transaction fee
