...
import PWCore, {
    ...
    Amount,
    AddressType,
    IndexerCollector, CellDep, DepType, OutPoint, Script, HashType,
    AmountUnit,
    SUDT,
} from './src';
import { DummyCollector } from './src/collectors/dummy-collector';
...
async function main() {
    ...
    const privateKey = '0xa800c82df5461756ae99b5c6677d019c98cc98c7786b80d7b2e77256e46ea1fe';
    ...
    const cells = await collector.collect(provider.address, {neededAmount: new Amount('1', AmountUnit.ckb)});
    console.dir(cells, {depth: null});

    const sudt = new SUDT('0x2d55f9ce00eb466459c561f9b889abc5b4cdfbfcace0edfa67b68abc4b8cbe4f');
    const sudtBalance = await collector.getSUDTBalance( sudt, provider.address);
    console.dir(sudtBalance, {depth: null});

    const sudtCells = await collector.collectSUDT(
        sudt,
        provider.address,
    ...
    const toAddr = new Address('ckt1qyq9ed70kzznd7alyjt7dhcq3kvl5sj2sdmsgflp40', AddressType.ckb);
